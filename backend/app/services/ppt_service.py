...
def save_results_to_txt(
    ...
    """
    추출 결과를 result 폴더에 .txt 파일로 저장.
    - 전체 합본: result/<파일명>.txt
    - 슬라이드별: result/<파일명>_slide_<번호>.txt

    Returns:
        (합본 경로, 슬라이드별 경로 목록)
    """
    base_name = input_path.stem
    out_dir = result_dir or (input_path.parent / "result")
    ...
    for slide_data in results:
        ...
        txt = format_slide_as_text(slide_data)
        slide_path = out_dir / f"{base_name}_slide_{num:03d}.txt"
        slide_path.write_text(txt, encoding="utf-8")
        slide_paths.append(slide_path)

    # 전체 합본 파일
    full_txt = "\n".join(format_slide_as_text(s) for s in results)
    full_path = out_dir / f"{base_name}.txt"
    full_path.write_text(full_txt, encoding="utf-8")

    return full_path, slide_paths
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PPT/PPTX 텍스트 추출기")
    parser.add_argument("input", help="입력 .pptx 파일 경로")
    parser.add_argument(
        "-o", "--output", default=None, help="출력 JSON 파일 경로 (기본: result/<파일명>.json)"
    )
    parser.add_argument(
        "-r", "--result-dir", default=None, help="TXT 저장 폴더 (기본: 입력파일 옆 result/)"
    )
    parser.add_argument(
        "-c",
        "--confidence",
        ...
        help=f"OCR 신뢰도 최소값 (기본: {DEFAULT_CONFIDENCE_THRESHOLD})",
    )
    parser.add_argument(
        "--slides",
        type=str,
        default=None,
        help="처리할 슬라이드 범위 (예: 1-5). 기본: 전체",
    )
    parser.add_argument(
        "--no-json", action="store_true", help="JSON 파일을 생성하지 않음"
    )
    ...
    result_dir = Path(args.result_dir) if args.result_dir else input_path.parent / "result"
    ...
    print(f"추출 시작: {input_path}")
    results = extract_text_from_pptx(input_path, args.confidence, slide_range)

    # TXT 저장
    full_path, slide_paths = save_results_to_txt(results, input_path, result_dir)
    print(f"TXT 합본  : {full_path}")
    for p in slide_paths:
        print(f"  슬라이드 : {p.name}")

    # JSON 저장 (--no-json 플래그가 없으면 기본 저장)
    if not args.no_json:
        json_path = (
            Path(args.output)
            if args.output
            else result_dir / f"{input_path.stem}.json"
        )
    ...
    print(f"\n완료: {len(results)}개 슬라이드 → {result_dir}")
